...
public class pb01_P5_1708_블록껍질2 {
	...
	public static void main(String[] args) throws Exception {
		...
		ArrayList<Point> pa=new ArrayList<>(); // 점들을 저장할 배열, 점의개수가 100_000이므로 collectionsort를 이용하기위해 리스트배열을 사용함
		...
		for (int i = 0; i < pa.size(); i++) {
            if (pa.get(i).y < root.y) {
                root = pa.get(i);
            } else if (pa.get(i).y == root.y) {
                if (pa.get(i).x < root.x) {
                    root = pa.get(i);
                }
            }
        }
		
		pa.sort(new Comparator<Point>() {
			@Override
